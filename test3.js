...
function updateMesh() {
    ...
    var v = config.plane.anchor.position.clone();
...
var Config = function() {
    ...
    this.updateVerticies = function() {
        var index = Math.floor(Math.random()*config.plane.geometry.vertices.length);
        config.plane.geometry.vertices[index].x += (Math.random()*this.noise-this.noise/2);
        config.plane.geometry.vertices[index].y += (Math.random()*this.noise-this.noise/2);
        config.plane.geometry.vertices[index].z += (Math.random()-0.5);
        config.plane.geometry.elementsNeedUpdate = true;
    };
    this.offsetX = 0;
    this.offsetY = 0;
    this.labelScale = 1;
...
window.addEventListener('load', function() {
    var animate = function(){
        ...
        // config.plane
        // updateMesh();
        config.plane.position.copy(config.plane.anchor.position);
        var e = config.plane.rotAtAnchor;
        var E  = new THREE.Euler( e[0], e[1], e[2], "XYZ" );
        // console.log("before:"+config.plane.quaternion);
        config.plane.quaternion.setFromEuler(E); // shit
        // console.log("after:"+config.plane.quaternion);
...
gui.add(config, 'offsetX').min(-1).max(1).step(0.01).onChange(function(value){
    config.plane.material.map.offset.x = value;
}).name('Offset X');
gui.add(config, 'offsetY').min(-1).max(1).step(0.01).onChange(function(value){
    config.plane.material.map.offset.y = value;
}).name('Offset Y');
gui.add(config, 'labelScale').min(0.1).max(10).step(0.1).onChange(function(value){
    // config.spritey.position;
    config.spritey.scale.x = config.spritey.scaleX * value;
    config.spritey.scale.y = config.spritey.scaleY * value;
    /*
    var v = config.spritey.position;
    var T1 = new THREE.Matrix4().makeTranslation( v.x,  v.y,  v.z);
    var T2 = new THREE.Matrix4().makeTranslation(-v.x, -v.y, -v.z);
    var S  = new THREE.Matrix4().makeScale(value, value, 1);
    T1.multiply(S);
    T1.multiply(T2);
    config.spritey.applyMatrix(T1);
    */
}).name('Label Scale');
gui.add(config, 'groupRotX').min(0).max(360).step(1).onChange(function(value){
    var delta = value/360*2*Math.PI;
    config.plane.rotAtAnchor[0] = delta;
}).name('groupRotX');
gui.add(config, 'groupRotY').min(0).max(360).step(1).onChange(function(value){
    var delta = value/360*2*Math.PI;
    config.plane.rotAtAnchor[1] = delta;
}).name('groupRotY');
gui.add(config, 'groupRotZ').min(0).max(360).step(1).onChange(function(value){
    var delta = value/360*2*Math.PI;
